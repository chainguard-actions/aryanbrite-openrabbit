...
function mapLineToPosition(patch, targetLine) {
    if (!patch)
        return null;
    let position = 0;
...
function mapLineToPositionWithContent(patch, targetLine, fileContent) {
    if (!patch)
        return null;
    const lines = patch.split(/\r?\n/);
    let position = 0;
...
const REVIEW_LENS_VALUES = ['default', 'security', 'socratic', 'performance', 'scope-guard'];
const REVIEW_LENS_INSTRUCTIONS = {
    default: '',
...
const promptTemplate = ({ title, body, linkedIssues, repositoryFiles, changedFiles, skippedFiles, reviewMode, toneMode, additionalFiles, specialInstructions, reviewLensInstructions, languageLenses, multiPassContext, priorSummaries, metadataNote, includePatches, }) => `You are an expert code reviewer embedded in a GitHub Action. Your job is to review pull requests with deep technical understanding, sharp judgment, and a human tone. You are NOT a linter. You think before you speak.
...
function getReviewLensInstructions(value) {
...
function buildLanguageLenses(changedFiles) {
...
function totalPatchLines(changedFiles) {
...
function buildReviewGroups(changedFiles) {
...
function normalizeComment(comment) {
    ...
    const type = typeof comment.type === 'string' && VALID_COMMENT_TYPES.includes(comment.type)
        ? comment.type
        : undefined;
...
export function buildReviewPrompt({ title, body, linkedIssues = [], repositoryFiles = [], changedFiles, skippedFiles = [], reviewMode = 'both', toneMode = 'balanced', additionalFiles, specialInstructions, reviewLensInstructions, languageLenses, multiPassContext, priorSummaries, metadataNote, includePatches = true, }) {
...
export function parseReviewResponse(text) {
    ...
    try {
        ...
        const commentsWithNull = Array.isArray(parsed.comments)
            ? parsed.comments.map((item) => normalizeComment(item))
            : [];
        const comments = commentsWithNull.filter((comment) => Boolean(comment));
...
function extractLinkedIssueNumbers(owner, repo, body) {
    if (!body) {
        return [];
    }
...
async function fetchLinkedIssues(octokit, owner, repo, body) {
...
function renderSummaryMarkdown(summary, separatePrSuggestions) {
    const sections = [];
    if (summary.verdict) {
...
function buildPassSummary(label, summary, separatePrSuggestions) {
    const heading = `### ${label}`;
...
export async function runReview(context) {
    ...
    const manifestOrLock = (p) => /(^|\/)package\.json$|(^|\/)package-lock\.json$|(^|\/)yarn\.lock$|(^|\/)pnpm-lock\.yaml$/i.test(p);
    ...
    async function runReviewPass(options) {
        const { title, body, changedFiles, reviewMode, toneMode, additionalFiles, includePatches, multiPassContext, priorSummaries, metadataNote, languageLenses, } = options;
    ...
    async function suggestionLooksRelevant(path, suggestion) {
        try {
            const { data } = await octokit.rest.repos.getContent({ owner: context.owner, repo: context.repo, path, ref: pullRequest.head.sha });
            if (!('content' in data) || typeof data.content !== 'string')
                return false;
            const raw = Buffer.from(data.content, 'base64').toString('utf8').toLowerCase();
    ...
    await octokit.rest.pulls.createReview(createParams);
}
